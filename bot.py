...
async def auto_post_loop(user_id, phone, session_str):
    ...
    logger.info(f"🔄 بدء النشر التلقائي {phone}")
    while True:
        try:
            ...
            if not acc_settings.get("enabled", False):
                logger.info(f"⏹️ إيقاف النشر {phone}")
                break
            ...
            if not await temp.is_user_authorized():
                logger.warning(f"❌ {phone} غير مصرح")
            ...
            if groups:
                ...
                for group in groups[:30]:
                    ...
                    try:
                        ...
                        logger.warning(f"⏱️ {phone}: انتظار {e.seconds}")
                        await asyncio.sleep(e.seconds)
                    except Exception as e:
                        logger.error(f"⚠️ {phone}: {e}")
            ...
            logger.error(f"❌ خطأ في النشر {phone}: {e}")
...
async def start_cmd(event):
    try:
        ...
        logger.info(f"📩 /start من {user_id}")
        ...
        await event.respond("⚠️ حدث خطأ، حاول مرة أخرى.")
...
async def manage_accounts(event):
    try:
        ...
        logger.error(f"❌ manage_accounts: {e}")
...
async def manage_single_account(event):
    try:
        ...
        logger.error(f"❌ manage_single_account: {e}")
...
async def fetch_from_account(event):
    try:
        ...
        logger.error(f"❌ fetch_from_account: {e}")
...
async def turbo_publish(event):
    try:
        ...
        logger.error(f"❌ turbo_publish: {e}")
...
async def publish_engine(event):
    try:
        ...
        logger.error(f"❌ publish_engine: {e}")
...
async def normal_publish(event):
    try:
        ...
        logger.error(f"❌ normal_publish: {e}")
...
async def auto_publish(event):
    try:
        ...
        await event.edit("🔄 **النشر التلقائي**\nاختر الحساب للتفعيل:", buttons=[
            [Button.inline(f"📱 {a.get('phone', a.get('phone_number', ''))}", f"toggle_{a.get('phone', a.get('phone_number', ''))}".encode()) for a in accounts[:3]],
            [Button.inline("🔙 رجوع", b"back_main")]
        ])
    except Exception as e:
        logger.error(f"❌ auto_publish: {e}")
...
async def running_processes(event):
    try:
        ...
        logger.error(f"❌ running_processes: {e}")
...
async def admin_callbacks(event):
    try:
        ...
        logger.error(f"❌ admin_callbacks: {e}")
