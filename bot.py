...
# بوت النشر التلقائي الاحترافي - النسخة النهائية المتكاملة
# المطور: @Motazalkade
...
# ========== دوال قراءة وكتابة البيانات المتوافقة ==========
def get_data(key, default=None):
    """قراءة البيانات مع دعم الإصدارات القديمة"""
    try:
        if db.exists(key):
            return db.get(key)
    except:
...
def init_database():
    keys = ["users", "accounts_settings", "memberships", "pending_requests", "admins", "bot_enabled", "user_stats", "collected_links", "groups_count"]
    for key in keys:
...
async def auto_post_loop(user_id, phone, session_str):
    ...
    logger.info(f"🔄 بدء النشر التلقائي للحساب {phone}")
    ...
    while True:
        try:
            ...
            if not acc_settings.get("enabled", False):
                logger.info(f"⏹️ إيقاف النشر للحساب {phone}")
                break
            ...
            if not await temp.is_user_authorized():
                logger.warning(f"❌ الحساب {phone} غير مصرح")
            ...
            if groups:
                ...
                for group in groups[:30]:
                    ...
                    try:
                        ...
                        logger.info(f"✅ {phone}: تم النشر في {group.name}")
                        await asyncio.sleep(random.uniform(3, 7))
                    except FloodWaitError as e:
                        logger.warning(f"⏱️ {phone}: انتظار {e.seconds} ثانية")
                        await asyncio.sleep(e.seconds)
                    except Exception as e:
                        logger.error(f"⚠️ {phone}: خطأ في النشر - {e}")
            ...
            logger.error(f"❌ خطأ في حلقة النشر {phone}: {e}")
...
async def start_cmd(event):
    try:
        ...
        logger.info(f"📩 استلام /start من {user_id}")
        ...
        logger.error(f"❌ خطأ في start_cmd: {e}")
        await event.respond("⚠️ حدث خطأ، يرجى المحاولة مرة أخرى.")
...
async def request_subscription(event):
    try:
        ...
        logger.error(f"❌ خطأ في request_sub: {e}")
...
async def accept_subscription(event):
    try:
        ...
        logger.error(f"❌ خطأ في accept_sub: {e}")
...
async def reject_subscription(event):
    try:
        ...
        logger.error(f"❌ خطأ في reject_sub: {e}")
...
async def add_account(event):
    try:
        ...
        logger.error(f"❌ خطأ في add_account: {e}")
        await event.answer("حدث خطأ، يرجى المحاولة مرة أخرى.", alert=True)
...
async def manage_accounts(event):
    try:
        ...
        logger.error(f"❌ خطأ في manage_accounts: {e}")
...
async def manage_single_account(event):
    try:
        ...
        logger.error(f"❌ خطأ في manage_single_account: {e}")
...
async def get_groups(event):
    try:
        ...
        logger.error(f"❌ خطأ في get_groups: {e}")
...
async def set_message(event):
    try:
        ...
        logger.error(f"❌ خطأ في set_message: {e}")
...
async def set_interval(event):
    try:
        ...
        logger.error(f"❌ خطأ في set_interval: {e}")
...
async def toggle_post(event):
    try:
        ...
        logger.error(f"❌ خطأ في toggle_post: {e}")
...
async def fetch_links(event):
    try:
        ...
        logger.error(f"❌ خطأ في fetch_links: {e}")
...
async def fetch_from_account(event):
    try:
        ...
        logger.error(f"❌ خطأ في fetch_from_account: {e}")
...
async def export_links(event):
    try:
        ...
        logger.error(f"❌ خطأ في export_links: {e}")
...
async def turbo_publish(event):
    try:
        ...
        logger.error(f"❌ خطأ في turbo_publish: {e}")
...
async def publish_engine(event):
    try:
        ...
        logger.error(f"❌ خطأ في publish_engine: {e}")
...
async def normal_publish(event):
    try:
        ...
        logger.error(f"❌ خطأ في normal_publish: {e}")

@client.on(events.CallbackQuery(data=b"auto_publish"))
async def auto_publish(event):
    try:
        user_id = event.chat_id
        users = get_data("users")
        accounts = users.get(str(user_id), {}).get("accounts", [])
        ...
        await event.edit("🔄 **النشر التلقائي**\nاختر الحساب للتفعيل:", buttons=[
            [Button.inline(f"📱 {a.get('phone')}", f"toggle_{a.get('phone')}".encode()) for a in accounts[:3]],
            [Button.inline("🔙 رجوع", b"back_main")]
        ])
    except Exception as e:
...
async def running_processes(event):
    try:
        ...
        logger.error(f"❌ خطأ في running_processes: {e}")
...
async def help_bot(event):
    try:
        ...
        logger.error(f"❌ خطأ في help_bot: {e}")
...
async def admin_callbacks(event):
    try:
        ...
        logger.error(f"❌ خطأ في admin_callbacks: {e}")
...
# ========== تشغيل البوت ==========
logger.info("✅ البوت شغال @Motazalkade")
